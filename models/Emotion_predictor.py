...
class Predictor:
    ...
    async def predict(self, image):
        ...
        probs, best_prob, predicted_id, predicted_class = self.predict_emotion(output)
    ...
    def load_model(self):
        try:
            model = EmotionModel(EmotionDataConfig.N_CLASSES)
    ...
    def create_transform(self):
        self.transforms_ = torchvision.transforms.Compose([
            ...
            transforms.CenterCrop(EmotionDataConfig.IMG_SIZE),
            transforms.ToTensor(),
            transforms.Normalize(mean = EmotionDataConfig.NORMALIZE_MEAN, std = EmotionDataConfig.NORMALIZE_STD)
        ])
    
    def predict_emotion(self, output):
        probabilities = F.softmax(output, dim = 1)
        best_prob = torch.max(probabilities, 1)[0].item()
        predicted_id = torch.max(probabilities, 1)[1].item()
        predicted_class = EmotionDataConfig.ID8DLABEL[predicted_id]
